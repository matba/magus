...
function editModalDataClass(featureModel,featureUUID){
	...
	editModalData.loadedFeatureFMRef = featureModel.findFeaturebyUUID(featureUUID) ;
	editModalData.loadedFeatureFMVisRef = editfeatureModelVis.findFeaturebyUUID(featureUUID) ;
	editModalData.entities= featureModel.annotations[featureUUID].entities.slice(0);
	editModalData.preconditions= featureModel.annotations[featureUUID].preconditions.slice(0);
	editModalData.effects= featureModel.annotations[featureUUID].effects.slice(0);
...
function loadFMAnnotationDialog(featureUUID){
	...
	$('#featureEdit_featureNameInput').val(editModalData.loadedFeatureFMRef.name);
...
function fillFMAnnotationDialog(editModalData){
	...
	$.each(entitiesList,function( index, value ) {
		$( "#featureEdit_entitiesList" ).append( '<span style="display: inline-block;" class="label '+classes[index%3]+'">'+editFeatureModel.toStringEntity(value)+'&nbsp;<a href="#" style="color:white" onclick="removeAnnotation(\'entity\','+index+');">x</a></span>&nbsp;' );
	})
	$('#featureEdit_preconditionsList').empty();
	$.each(pfactList,function( index, value ) {
		$( "#featureEdit_preconditionsList" ).append( '<span style="display: inline-block;" class="label '+classes[(index+1)%3]+'">'+editFeatureModel.toStringFact(value)+'&nbsp;<a href="#" style="color:white" onclick="removeAnnotation(\'precondition\','+index+');">x</a></span>&nbsp;' );
	})
	$('#featureEdit_effectsList').empty();
	$.each(efactList,function( index, value ) {
		$( "#featureEdit_effectsList" ).append( '<span style="display: inline-block;" class="label '+classes[(index+2)%3]+'">'+editFeatureModel.toStringFact(value)+'&nbsp;<a href="#" style="color:white" onclick="removeAnnotation(\'effect\','+index+');">x</a></span>&nbsp;' );
	});
	...
	if(type=='entity'){
		...
		$("#featureEdit_entityAddDiv").show();
			...
			$.each(varList,function( index, value ) {
				varData.push({label: value.name.name, value:value.name.uri});
			});
			var firstEntityInput = document.getElementById("featureEdit_firstEntityInput");
			...
			var secondEntityInput = document.getElementById("featureEdit_secondEntityInput");
			...
			if(type=='precondition'){
				...
				$( "#featureEdit_addFact" ).click({param1: "precondition"},addFact)
			}
			if(type=='effect'){
				hideEntityAdd();
				$("#featureEdit_factAddDiv").show();
				
				$( "#featureEdit_addFact" ).click({param1: "effect"},addFact)
			}
		}
}
...
function updateFMFeature(){
	editModalData.loadedFeatureFMRef.name = $('#featureEdit_featureNameInput').val();
	editModalData.loadedFeatureFMVisRef.l = $('#featureEdit_featureNameInput').val();
...
function addEntity(){
	...
	function validateEntityName(){
		...
		if(editFeatureModel.getEntityNamesList().indexOf($('#featureEdit_entityInput').val())!=-1){
	...
	if(validateEntityType()&&validateEntityName()){
		$("#featureEdit_Alert").hide();
		var newEntity = {name:{name:$('#featureEdit_entityInput').val(), uri:editFeatureModel.baseURI+"#"+$('#featureEdit_entityInput').val()}, type:{name: utility.getEntityFragment( $('#featureEdit_entityTypeInput').val()) ,uri:  $('#featureEdit_entityTypeInput').val()}};
...
function getCurrentStatusVarList(){
	...
	var otherFeatureEntities = editFeatureModel.getEntitiesExceptForFeature(editModalData.loadFeatureUUID);
	return  contextModel.instances.concat(otherFeatureEntities.concat(currentFeatureEntities)); 
}
...
function addFact(event){
	...
	function validateFirstEntity(){
		...
		var factType = contextModel.lookupFactType( $('#featureEdit_factTypeInput').val()).arguments[0].uri;
	...
	function validateSecondEntity(){
		...
		var controlValue = $(controlId).val();
		var factType = contextModel.lookupFactType( $('#featureEdit_factTypeInput').val()).arguments[1].uri;
		return validateFactEntityName(controlValue,factType,controlId);
	}
	...
	if(validateFactType()&&validateFirstEntity()&&validateSecondEntity()){
		$("#featureEdit_Alert").hide();
		var newFactType ={name: utility.getEntityFragment( $('#featureEdit_factTypeInput').val()) ,uri:  $('#featureEdit_factTypeInput').val()};
		var newArgument1 = {name: utility.getEntityFragment( $('#featureEdit_firstEntityInput').val()) ,uri:  $('#featureEdit_firstEntityInput').val()};
		var newArgument2 = {name: utility.getEntityFragment( $('#featureEdit_secondEntityInput').val()) ,uri:  $('#featureEdit_secondEntityInput').val()};
		
		newFact ={fact:newFactType, arguments:[newArgument1,newArgument2]};
		if(type=='precondition')
...
function addChild(type,featureUUID,featureNo ){
	if(type=='mandatory'){
		uuid = editFeatureModel.addChild(featureUUID,'New Feature',false,false,false );
		editfeatureModelVis.addLeaf(featureNo,'New Feature',false,false,false,uuid);
		
	}
	if(type=='optional'){
		uuid =editFeatureModel.addChild(featureUUID,'New Feature',true,false,false );
		editfeatureModelVis.addLeaf(featureNo,'New Feature',true,false,true,uuid );
	}
	if(type=='alternative'){
		
		editFeatureModel.updateFeature(featureUUID,true,false);
		uuid1 =editFeatureModel.addChild(featureUUID,'Feature 1',false,false,false);
		uuid2 =editFeatureModel.addChild(featureUUID,'Feature 2',false,false,false);
		editfeatureModelVis.updateFeature(featureNo,true,false); 
		editfeatureModelVis.addLeaf(featureNo,'Feature 1',false,false,false,uuid1);
		editfeatureModelVis.addLeaf(featureNo,'Feature 2',false,false,false,uuid2);
	}
	if(type=='orgroup'){
...
function downloadFMSVG(){
	var svgcontent =$("<div />").append($("#featureModelVissvg").clone()).html();  
...
function adaptMashup(serviceIndex, isAvailable){
	if(!isAvailable){
		...
		logger.addToLog('Service has become unavailable. Starting adaptation ...');
		var doc = configurationFeatureModel.serializeToXML();
		var serializer = new XMLSerializer();
		var xmlString = serializer.serializeToString(doc);
		
		var contextModelSerialized = serializer.serializeToString(contextModel.serializeToXML());
		...
		$.ajax({
			...
			.done(function( data ) {
			    ...
			    if(data.adaptationType==-1){
			    	...
			    	runningFMDistance =0;
			    	clearResultTabs();
			    	utility.showMessage('Sorry','Something went wrong.');
			    }
			    ...
			    if(data.adaptationType==-2){
			    	...
			    	clearResultTabs();
			    	utility.showMessage('Adaptation Result','The service mashup failure <b> <span class="label label-danger">Can Not Be Addressed</span> </b> by adaptation.');
			    }
			    ...
			    if(data.adaptationType==0){
			    	...
			    	var newdoc =  CodeMirror.Doc(data.bpelXML, "xml");
					...
					$.each(usedMashupServices,function( index, value ) {
						if(value==serviceList[serviceIndex].svs.baseURI)
							usedMashupServices[index] = data.replacementServiceURI;
					});
					
					utility.showMessage('Adaptation Result','The service mashup functionality <b> <span class="label label-success">Recovered</span> </b> by adaptation through <b><span class="label label-success"><span class="glyphicon glyphicon-sort" aria-hidden="true"></span> Service Replacement</span></b>. The failed service <b>'+serviceList[serviceIndex].svs.name+'</b> has been replaced with <b>'+data.replacementServiceName+'</b>.Please see the result tabs for updated process.');
			    ...
			    if(data.adaptationType==1){
					...
					utility.showMessage('Adaptation Result','The service mashup functionality <b> <span class="label label-success">Recovered</span> </b> by adaptation through <b><span class="label label-success"><span class="glyphicon glyphicon-random" aria-hidden="true"></span> Replanning</span></b>. Please see the result tabs for updated process.');
			...
			.fail(function() {
			    logger.addToLog( "Error in calling Adapt Service Mashup Service." );
			    $('#pleaseWaitModal').modal('hide');
			  });
		...
		$('#pleaseWaitModal').modal('show');
		logger.addToLog('A Service has become available. Starting the investigation if the service mashup can be improved ...');
		var doc = configurationFeatureModel.serializeToXML();
		var serializer = new XMLSerializer();
		var xmlString = serializer.serializeToString(doc);
			...
			.done(function( data ) {
			    ...
			    if((data.adaptationType==2)){
			    	...
			    	if((runningFMDistance > data.alternateFMDistance)){
			    		...
			    		logger.addToLog("The mashup cannot be improved using the service has newly become available.");
			    	}
			    }
...
function generateMashup(){
	...
	var validationResult = configurationFeatureModel.validateConfiguration(selectedFeatures);
	...
	if(!validationResult.success){
		...
		$.ajax({
			...
			.done(function( data ) {
			    ...
			    if((data.workflowJSON!=null )&&( data.workflowJSON.length>0)){
			    	workflowGraphJSON =data.workflowJSON
			    	var graph =eval("(" + data.workflowJSON + ")");
			    	//workflowGraphJSON=graph;
			    	//console.log(graph);
			    ...
			    if((data.bpelXML!=null )&&(data.bpelXML.length>0)){
			    	var newdoc =  CodeMirror.Doc(data.bpelXML, "xml");
					bpelCodeMirror.swapDoc(newdoc);
					bpelXMLDoc =data.bpelXML;
			...
			.fail(function() {
			    logger.addToLog( "Error in calling Generate Service Mashup Service." );
			    $('#pleaseWaitModal').modal('hide');
			  });
	...
	Starting generating service mashup for feature model configuration....
	Validating the feature model configuration...DONE(54ms)
	Starting reduction to planning...
	Preparing planning domain...DONE(40ms, 22 actions)
	Converting the planning problem...DONE(0ms, 9 variables, 1 initial states, 5 goal states)
	Starting FFPlanner...
	Solution found (200ms, 7 operation)
	Starting optimizing solution...
	Solution optimization done (450ms).
	Generating BPEL code...DONE(4ms, 123 lines)
	Finished generating service mashup for feature model configuration.
