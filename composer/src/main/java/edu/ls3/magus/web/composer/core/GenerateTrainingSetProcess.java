...
public class GenerateTrainingSetProcess extends Process {
	...
	public void process() throws Exception {
		...
		File trainingSetDirectory = new File(familyDirectory + File.separator + Configuration.trainingSetDirectory);
		...
		List<FeatureModelConfiguration> adequateTrainingList = domainModel.getFeatureModel()
				.generateRegressionConfigurations(1, fasm);
		...
		for (FeatureModelConfiguration fmc : flows.keySet()) {
			...
			String features = fmc.getFeatureList().stream().map(value -> value.getUuid())
			...
			UtilityClass.writeFile(
					new File(trainingSetDirectory.getAbsolutePath() + File.separator + "FM" + featureModelUuid),
					flows.get(fmc));

		}

		Set<AtomicSet> atomicSets = fasm.getFasMap().values().stream().collect(Collectors.toSet());
		UtilityClass.writeFile(new File(trainingSetDirectory.getAbsolutePath() + File.separator + "fasm"),
				AtomicSet.serializeToXml(atomicSets));
		UtilityClass.writeFile(
				new File(trainingSetDirectory.getAbsolutePath() + File.separator + Configuration.configurationFileName),
				configurationFileBuilder.toString());
