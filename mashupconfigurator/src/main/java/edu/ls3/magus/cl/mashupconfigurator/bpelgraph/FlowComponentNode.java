...
public class FlowComponentNode extends ComponentNode {
	...
	public void OptimizeNo2() {
		...
		while (maximumLinks < getLinks().size()) {
			...
			if (candidates.isEmpty()) {
				...
				// try {
				// edu.ls3.magus.eval.generators.owls.UtilityClass.writeFile(new File(
				// "/home/mbashari/intermediate/testblock"+cntr+".gv"),s);
				// } catch (IOException e) {
				//
				// e.printStackTrace();
				// }
	...
	private void FoldComponent(FoldingCandidate fc) {
		...
		if (cn.getClass().equals(FlowComponentNode.class)) {
			...
			for (int cnt = 0; cnt < cn.getNodes().size(); cnt++) {
				...
				// if((nf.getClass().equals(FlowComponentNode.class))&&((FlowComponentNode)
				// nf).getNodes().contains(cn.getNodes().get(cnt)))
				// {
			...
			// for(Link l: getLinks()){
			//
			// if(l.getStartNode().equals(startNode) &&
			// l.getEndNode().equals(flowSubComs.get(cnt1))){
			// toberemoved.add(l);
			//
			// }
			// if(l.getStartNode().equals(flowSubComs.get(cnt1)) &&
			// l.getEndNode().equals(endNode)){
			// toberemoved.add(l);
			//
			// }
			// }
		...
		if (cn.getClass().equals(SequenceComponentNode.class)) {
			...
			// for(Node n: getNodes())
			// if((n.getClass().equals(SequenceComponentNode.class))&&((SequenceComponentNode)
			// n).getNodes().contains(cn.getNodes().get(cnt)))
			// {
			...
			for (int cnt = 0; cnt < cn.getNodes().size() - 1; cnt++) {
				for (Link l : getLinks())
					if (l.getStartNode().equals(cn.getNodes().get(cnt))
							&& l.getEndNode().equals(cn.getNodes().get(cnt + 1))) {
						getLinks().remove(l);
						break;
					}
			}
	...
	private List<FoldingCandidate> getSequenceFoldingCandidate() {
		...
		while (!nodes.isEmpty()) {
			...
			if ((precedingNodes.size() < 2) && (followingNodes.size() != 1))
				scn.getNodes().add(curNode);
			else if ((precedingNodes.size() < 2) && (followingNodes.size() == 0))
				scn.getNodes().add(curNode);
	...
	// if((precedingNodes.size()!=pNodePrecedingNode.size()) ||
	// !UtilityClass.includesAll(pNodePrecedingNode, precedingNodes))
	// continue;
	//
	// if((followingNodes.size()!=pNodeFollowingNodes.size()) ||
	// !UtilityClass.includesAll(pNodeFollowingNodes,followingNodes))
	// continue;
	//
	//
	// survivedNodes.add(n);
	...
	private List<FoldingCandidate> getFlowFoldingCandidateFixed() {
		...
		while (!nodes.isEmpty()) {
			...
			for (Node n : flowStartNodeFollowing) {
				...
				if ((followingNodes.size() != pNodeFollowingNodes.size()) || ((followingNodes.size() != 0)
						&& !UtilityClass.includesAll(pNodeFollowingNodes, followingNodes)))
					continue;
	...
	// continue;
	// List<FlowComponentNode> cfcn = GetAllFlows(curNode,enNodes,new
	// ArrayList<Node>(), intersection,maxinumLinks);
	...
	private List<FlowComponentNode> GetAllFlows(Node stNode, Node enNode, List<Node> inflowNodes,
			List<Node> intersection, int chancesLeft) {
	...
	public static Node createARandomFlow(int no, List<Service> usedServices, ServiceCollection sc) throws Exception {
		if (no < UtilityClass.CuttedRandValueNormal(3, 2, 2)) {
			...
			FlowComponentNode fcn = new FlowComponentNode();
			while (remaining > 0) {
				int nextbunch = (int) UtilityClass.CuttedRandValueNormal(no / noOfDevisions,
						Math.max(no / (noOfDevisions * 5), 1), 1);
				devisionCounter++;
			...
			for (int cnt = 0; cnt < NoOfLinks; cnt++) {
				...
				if (stNode instanceof ComponentNode) {
					astNode = ((ComponentNode) stNode).getNodes()
							.get(UtilityClass.randInt(0, ((ComponentNode) stNode).getNodes().size() - 2));
				}
				...
				if (enNode instanceof ComponentNode) {
					aenNode = ((ComponentNode) enNode).getNodes()
							.get(UtilityClass.randInt(1, ((ComponentNode) enNode).getNodes().size() - 1));
				}
	...
	public String serializeToBpel(List<Instance> vars) {
		...
		result.append(currentTab + "<!-- process1 BPEL Process [Generated by the MAGUS Mashup tool]  -->"
		...
		result.append(currentTab + "<!-- ================================================================= -->"
				+ System.lineSeparator());
		result.append(currentTab + "<!-- PARTNERLINKS                                                      -->"
				+ System.lineSeparator());
		result.append(currentTab + "<!-- List of services participating in this BPEL process               -->"
				+ System.lineSeparator());
		result.append(currentTab + "<!-- ================================================================= -->"
				+ System.lineSeparator());
		...
		for (Service s : svs) {
			...
			result.append(currentTab + "\t<bpel:partnerLink name=\"" + s.getName() + "\"  partnerRole=\"" + s.getName()
					+ "Service\" " + cbStr + "  partnerLinkType=\"" + s.getURI() + "\"/>" + System.lineSeparator());
		}
		...
		result.append(currentTab + "<!-- ================================================================= -->"
				+ System.lineSeparator());
		result.append(currentTab + "<!-- VARIABLES                                                         -->"
				+ System.lineSeparator());

		result.append(currentTab + "<!-- List of messages and XML documents used within this BPEL process  -->"
				+ System.lineSeparator());
		result.append(currentTab + "<!-- ================================================================= --> "
				+ System.lineSeparator());
		...
		for (Instance v : vars)
			result.append(currentTab + "\t<bpel:variable name=\"" + v.getName() + "\"  messageType=\"" + v.getURI()
					+ "\" />" + System.lineSeparator());
		...
		result.append(currentTab + "<!-- ================================================================= -->"
				+ System.lineSeparator());
		result.append(currentTab + "<!-- ORCHESTRATION LOGIC                                               -->"
				+ System.lineSeparator());
		result.append(currentTab + "<!-- Set of activities coordinating the flow of messages across the    -->"
				+ System.lineSeparator());
		result.append(currentTab + "<!-- services integrated within this business process                  -->"
				+ System.lineSeparator());
		result.append(currentTab + "<!-- ================================================================= -->"
				+ System.lineSeparator());
		...
		result.append(currentTab + "\t" + "<bpel:receive operation=\"--\"   variable=\"" + inputs
				+ "\" createInstance=\"yes\" />" + System.lineSeparator());
		...
		result.append(currentTab + "\t" + "<bpel:reply operation=\"--\"   variable=\"" + outputs + "\" />"
				+ System.lineSeparator());
	...
	public List<Service> getAllInvokedServices() throws Exception {
		List<Service> result = new ArrayList<>();
		if (getNodes().isEmpty())
			return result;

		List<OperationNode> graph = convertToWorkflow();

		for (OperationNode on : graph) {
			if (on.getCalledService() != null && on.getCalledService().getCalledService() != null
					&& (on.getCalledService().getCalledService().getInvocationService() == null)
					&& (on.getCalledService().getCalledService().getURI() != null)
					&& (on.getCalledService().getCalledService().getURI() != ""))
				result.add(on.getCalledService().getCalledService());
		}
