...
public class ExecutionTimeType implements NonfunctionalMetricType {
		...
		while(!q.isEmpty()){
			...
			for(OperationNode gn: curNode.getEdges())
			{
				...
				if(allDone)
				{
					...
					if(!gn.isEndNode())
						timeAfterExecution+=((NonfunctionalMetric) map.get(gn.getCalledService().getCalledService()).getAnnotation().get(this)).getAverage();
	...
	public static double findExecutionTime(Map<Service, ServiceNonfunctionalAnnotation> map, List<OperationNode> graph){
		List<OperationNode> doneNodes= new ArrayList<OperationNode>();
		Map<OperationNode, Double> executionTime = new HashMap<OperationNode, Double>();
		OperationNode startNode=null;
		for(OperationNode gn: graph)
				...
				if(allDone)
				{
					...
					for(OperationNode gn2: preNodes)
						if((timeAfterExecution==-1d)||(timeAfterExecution<executionTime.get(gn2)))
								timeAfterExecution= executionTime.get(gn2);
					if(!gn.isEndNode())
						timeAfterExecution+=((NonfunctionalMetric) map.get(gn.getCalledService().getCalledService()).getAnnotation().get("EXECUTION_TIME")).getExecutionValue();
					executionTime.put(gn, timeAfterExecution);
